...
const createShortUrlHandler = async (data) => {
    ...
    const shortenUrl= `${import.meta.env.VITE_FRONT_END_URL + "/s/" + `${response.shortUrl}`}`;
  ...
  return (
    <div className='flex justify-center items-center bg-white rounded-md'>
        ...
        className='sm:w-[450px] w-[360px] relative shadow-custom pt-8 pb-5 sm:px-8 px-4 rounded-lg'>
            ...
            <hr className='mt-2 sm:mb-5 mb-3 text-slate-900'/>
            <div>
                <TextField
                label="Enter URL"
                required
                ...
                {loading && (
                    <Tooltip title="Close">
                        ...
                        className='absolute right-2 top-2 '>
                            <RxCross2 className='text-slate-900 text-3xl' /> 
                        </button>
                    </Tooltip>
                )}
